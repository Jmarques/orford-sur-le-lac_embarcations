...
- 2026-07-06 : la section « Quota » reportée nommée (exigence 5) est désormais spécifiée — décision 0019 et PRD `.scratch/hors-quota/PRD.md` (section « Hors quota », clé adresse, fiche d'adresse, notes d'adresse au Journal).
- 2026-07-06 (retour terrain de Jeremy, ~25 cas réels) : refonte UX — la page devient un **registre** de rangées compactes + une **fiche** (wa-dialog) qui porte le travail (membre, journal complet, ajout de note, mailto, libération) ; vocabulaire « **note** » / « **journal de l'emplacement** » remplace « intervention » (glossaire mis à jour, action Journal `note`, aucune migration : le backend n'avait jamais été déployé avec l'ancien code). Les symptômes « une seule observation », « Aucun membre inscrit » et historique absent venaient d'un backend non redéployé (l'API ne renvoyait ni `journal` ni `membres`) — la page le signale désormais en console.
