...
- **Revue visuelle = le delta seulement** (décision 0017) : `screenshots/` est committé — c'est la baseline. `npm run screenshots` compare à HEAD et liste les captures modifiées/nouvelles avec leurs artefacts de revue (`screenshots/.diff/`, gitignoré) : crops avant/après/différence par zone changée, tuiles pour une capture neuve — donner au subagent CES chemins-là, jamais les pleines pages (illisibles une fois réduites par l'API vision). « Aucune différence » = pas de revue visuelle. Changement intentionnel → committer les PNG avec le code. Boucle interne sur une page : `npm run screenshots -- --page structures` (verify reste complet).
