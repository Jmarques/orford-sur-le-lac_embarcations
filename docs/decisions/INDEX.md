...
0017 — Revue visuelle incrémentale : captures committées = baseline, npm run screenshots ne signale que le delta pixel (artefacts avant/différence dans screenshots/.diff/), revue sur le delta seulement ; port éphémère, --page pour la boucle interne (0016 réservé par une session parallèle) [tooling, agents, ui]
