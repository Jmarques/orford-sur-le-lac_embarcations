...
- **Génération parallèle, sauf les overlays et défilements** : la file scénario × viewport est épuisée par quelques pages concurrentes sur le même navigateur — le temps de run croît avec le site mais reste bien sous les ~3 min séquentielles d'origine (~1 min 45 aujourd'hui) ; le gate console, attribué par page, n'y perd rien. **Exception** (issue 04) : les captures d'overlay ou de défilement (`pleinVue || defiler || voir`) sont capturées en un flux séquentiel après le pool. Sous concurrence, l'ouverture d'un drawer/dialog, un scrollIntoView ou le re-clampage du scroll interne par le stitch fullPage ne se stabilisent pas au même sous-pixel selon l'ordonnancement des pages voisines — faux positifs bi-stables allant de ~62 px (bordures pointillées d'une grille défilée) à des dizaines de milliers de px (corps de drawer décalé). Un run entièrement séquentiel est propre : la concurrence est la seule cause, on ne séquentialise donc que cette classe. Le compromis assumé : le déterminisme du gate prime sur la vitesse brute.
